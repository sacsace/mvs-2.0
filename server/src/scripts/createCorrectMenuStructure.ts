...
async function createCorrectMenuStructure() {
  try {
    ...
    // 최상위 메뉴들 생성
    const topLevelMenus = [
      { name: '사용자 관리', icon: 'people', order_num: 1, url: '/users' },
      { name: '권한 관리', icon: 'security', order_num: 2, url: '/permissions' },
      { name: '메뉴 관리', icon: 'menu', order_num: 3, url: '/menus' },
      { name: '인보이스 관리', icon: 'receipt', order_num: 4, url: '/invoices' },
      { name: '지출결의서 관리', icon: 'account_balance_wallet', order_num: 5, url: '/expenses' }
    ];
    ...
    for (const menu of topLevelMenus) {
      ...
      `, {
        replacements: [menu.name, menu.icon, menu.order_num, menu.url]
      });
    }
    ...
    // 각 최상위 메뉴의 ID 조회
    const [userMenu] = await sequelize.query('SELECT menu_id FROM menu WHERE name = ? AND parent_id IS NULL', {
      replacements: ['사용자 관리']
    });
    const userMenuId = (userMenu[0] as any).menu_id;

    const [permissionMenu] = await sequelize.query('SELECT menu_id FROM menu WHERE name = ? AND parent_id IS NULL', {
      replacements: ['권한 관리']
    });
    const permissionMenuId = (permissionMenu[0] as any).menu_id;

    const [invoiceMenu] = await sequelize.query('SELECT menu_id FROM menu WHERE name = ? AND parent_id IS NULL', {
      replacements: ['인보이스 관리']
    });
    const invoiceMenuId = (invoiceMenu[0] as any).menu_id;

    const [expenseMenu] = await sequelize.query('SELECT menu_id FROM menu WHERE name = ? AND parent_id IS NULL', {
