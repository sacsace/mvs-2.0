...
async function initializeSystemData() {
  try {
    ...
    try {
      // 1. 기존 데이터 정리 (외래키 제약조건 순서에 맞게)
      console.log('🧹 기존 데이터 정리 중...');
      
      // 외래키를 참조하는 테이블부터 삭제
      await MenuPermission.destroy({ where: {}, transaction });
      console.log('  - 메뉴 권한 삭제 완료');
      
      await RolePermission.destroy({ where: {}, transaction });
      console.log('  - 역할 권한 삭제 완료');
      ...
      const company = await Company.create({
        ...
        representative_name: 'Administrator',
