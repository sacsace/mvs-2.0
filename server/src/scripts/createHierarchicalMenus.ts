...
async function createHierarchicalMenus() {
  try {
    ...
    const menuData = [
      ...
      { name: 'Invoice 관리', icon: 'receipt', order_num: 4, parent_id: null, url: '/invoices' },
      { name: '지출 결의서 관리', icon: 'account_balance_wallet', order_num: 5, parent_id: null, url: '/expenses' }
    ];
    ...
    const permissionSubMenus = [
      { name: '사용자 권한 관리', icon: 'person_add', order_num: 1, parent_id: permissionMenuId, url: '/permissions/users' },
      { name: '메뉴 권한 관리', icon: 'menu_book', order_num: 2, parent_id: permissionMenuId, url: '/permissions/menus' },
      { name: '역할 관리', icon: 'admin_panel_settings', order_num: 3, parent_id: permissionMenuId, url: '/permissions/roles' }
    ];
    ...
    const userSubMenus = [
      ...
      { name: '사용자 그룹 관리', icon: 'group', order_num: 3, parent_id: userMenuId, url: '/users/groups' }
    ];
    ...
    const [invoiceMenu] = await sequelize.query('SELECT menu_id FROM menu WHERE name = ?', {
      replacements: ['Invoice 관리']
    });
    const invoiceMenuId = (invoiceMenu[0] as any).menu_id;

    // Invoice 관리 하위 메뉴들
    const invoiceSubMenus = [
      { name: 'Invoice 목록', icon: 'list_alt', order_num: 1, parent_id: invoiceMenuId, url: '/invoices/list' },
      { name: 'Invoice 생성', icon: 'add_circle', order_num: 2, parent_id: invoiceMenuId, url: '/invoices/create' },
      { name: 'Invoice 승인', icon: 'approval', order_num: 3, parent_id: invoiceMenuId, url: '/invoices/approve' }
    ];

    // Invoice 관리 하위 메뉴들 삽입
    ...
    console.log('Invoice 관리 하위 메뉴들이 생성되었습니다.');

    // 지출 결의서 관리 메뉴 ID 조회
    const [expenseMenu] = await sequelize.query('SELECT menu_id FROM menu WHERE name = ?', {
      replacements: ['지출 결의서 관리']
    });
    const expenseMenuId = (expenseMenu[0] as any).menu_id;

    // 지출 결의서 관리 하위 메뉴들
    const expenseSubMenus = [
      { name: '지출 결의서 목록', icon: 'receipt_long', order_num: 1, parent_id: expenseMenuId, url: '/expenses/list' },
      { name: '지출 결의서 작성', icon: 'edit_note', order_num: 2, parent_id: expenseMenuId, url: '/expenses/create' },
      { name: '지출 승인', icon: 'verified', order_num: 3, parent_id: expenseMenuId, url: '/expenses/approve' }
    ];
    ...
    console.log('지출 결의서 관리 하위 메뉴들이 생성되었습니다.');
    ...
    console.error('계층 메뉴 생성 중 오류 발생:', error);
  } finally {
    await sequelize.close();
  }
}

createHierarchicalMenus(); 